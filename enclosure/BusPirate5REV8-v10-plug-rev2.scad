...
module sd_card_plug(){
    ...
    //0.8mm straight (2 print beads)
    //rest chamfered (wall_width-0.8)
    sd_plug_clearance=0.5;
    cover_y=2;
        //sd card
    translate([-((pcb_xy/2)-sd_offset_x)+(sd_plug_clearance/2),-1*((body_xy/2)-wall_width-eps)+(sd_plug_clearance/2),bottom_h-pcb_h-(sd_plug_clearance/2)]){
        cube(size=[sd_x-sd_plug_clearance,wall_width+sd_plug_clearance,sd_h-pcb_h-sd_plug_clearance], anchor=TOP+BACK+LEFT);
       
        translate([-(cover_y/2)-(sd_plug_clearance/2),-wall_width-(sd_plug_clearance/2),(sd_h-pcb_h-(sd_plug_clearance/2))/2])
        cuboid(
            [sd_x+cover_y,.8,sd_h], rounding=0.4,
            edges=[TOP+FRONT,TOP+RIGHT,TOP+LEFT,FRONT+RIGHT, FRONT+LEFT,BOTTOM+FRONT, BOTTOM+LEFT, BOTTOM+RIGHT ],
            anchor=BACK+LEFT+TOP,
            $fn=64
        );

        rotate([180,0,0])
        translate([-(cover_y/2)-(sd_plug_clearance/2),sd_plug_clearance/2,(sd_h+pcb_h+(sd_plug_clearance/2))/2])
        difference(){
        cuboid(
            [sd_x+cover_y,0.8,sd_h-(cover_y/2)+(sd_plug_clearance/2)-0.05], //rounding=0.5,
            //edges=[TOP+FRONT,TOP+RIGHT,TOP+LEFT,FRONT+RIGHT, FRONT+LEFT ],
            anchor=BACK+LEFT+TOP,
            $fn=64
        );      
        translate([(sd_x+cover_y)/2,.1,-(sd_h-0.6)]) cuboid([4,1,1.8], anchor=BOTTOM+BACK);
        }  
    }   
